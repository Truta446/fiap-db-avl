...
class EmployeeController(object):
  ...
  def find(self) -> List[EmployeeModel]:
    return self.db.display("", True)

  def find_by_name(self, name: str) -> Tuple[int, EmployeeModel]:
    return self.db.search(name=name)

  def create(self, data: EmployeeModel) -> None:
    self.db.insert(node=data)

  def update(self, data: EmployeeModel) -> None:
